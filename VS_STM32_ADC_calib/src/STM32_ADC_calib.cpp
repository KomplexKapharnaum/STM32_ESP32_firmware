    ...
    if (HAL_FLASH_OB_Unlock() == HAL_OK)
    {
      ...
      if (HAL_FLASHEx_OBErase() == HAL_OK)
      {
        ...
        if (HAL_FLASH_OB_Lock() == HAL_OK)
        {
          ...
          // cette instruction lance un reset mais enregistre la valeur dans l'option byte correctement en continue !
...
void setup()
{
  ...
  Serial1.print("new OB : ");
  Serial1.println(readCalibrationValue());
  delay(2000); // add delay to read OB value
}
